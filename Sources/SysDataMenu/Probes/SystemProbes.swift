...
struct SystemProbe: StorageProbe {
    func probe() async -> [StorageItem] {
        ...
        // Named one by one rather than as `/Library/Caches/*`, and only the
        // ones the system will actually let go: in there are data vaults and
        // restricted entries that not even root may remove. The glob reached
        // them, rm printed "Operation not permitted" for each, and a delete
        // that had removed everything it could was reported as a failure.
        let removableCaches = systemCaches.children(includeHidden: true).filter(Self.isRemovable)
        // Sized from the entries it will delete, not the whole folder: the
        // protected ones stay, and counting them promised space no delete
        // could give back.
        var cacheBytes: Int64 = 0
        var newest: Date?
        for entry in removableCaches {
            let measured = await DiskSize.measure(at: entry)
            cacheBytes += measured.bytes
            if let modified = measured.lastModified, modified > (newest ?? .distantPast) { newest = modified }
        }
        if !removableCaches.isEmpty, cacheBytes >= 10 * ProbeSupport.megabyte {
            items.append(StorageItem(
                ...
                sizeBytes: cacheBytes, safety: .safe,
                action: .privilegedScript(Self.removeScript(for: removableCaches)),
                revealURL: systemCaches, lastModified: newest
            ))
        }
        ...
        // Only redundant when an Xcode is installed and also the selected
        // developer directory. Selected themselves, they are what the
        // terminal is using, whatever else is in /Applications.
        let active = await ProbeSupport.activeDeveloperDirectory()
        let hasXcode = URL(fileURLWithPath: "/Applications").children()
            .contains { $0.lastPathComponent.hasPrefix("Xcode") && $0.pathExtension == "app" }
            && active.map { !$0.hasPrefix(commandLineTools.path) } == true
        if let item = await ProbeSupport.directoryItem(
    ...
    /// Whether anything on this Mac could remove this path.
    ///
    /// macOS protects some files from every user, root included: data vaults,
    /// and anything the system marks restricted. They cannot be deleted, and
    /// they cannot even be looked at — `lstat` returns "Operation not
    /// permitted" — which is the cheapest test there is. Listing directories
    /// sometimes hands them over and sometimes does not, so the check is made
    /// per entry rather than trusted to the enumeration.
    static func isRemovable(_ url: URL) -> Bool {
        var info = stat()
        guard lstat(url.path, &info) == 0 else { return false }
        let protectedFlags = UInt32(SF_RESTRICTED) | UInt32(UF_DATAVAULT)
        return info.st_flags & protectedFlags == 0
    }

    /// `rm -rf` over named paths rather than a glob, quoted for the shell the
    /// administrator prompt runs it in.
    static func removeScript(for urls: [URL]) -> String {
        "rm -rf " + urls.map { Shell.shellQuote($0.path) }.joined(separator: " ")
    }
}
