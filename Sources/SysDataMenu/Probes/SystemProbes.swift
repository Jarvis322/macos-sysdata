...
struct SystemProbe: StorageProbe {
    func probe() async -> [StorageItem] {
        ...
        // Named one by one rather than as `/Library/Caches/*`. The system
        // keeps entries in there that not even root may touch, and they are
        // not visible to this app either: the glob reached them, rm printed
        // "Operation not permitted" for each, and a delete that had removed
        // everything it could was reported as a failure. These are the
        // entries the app can see, which are the ones it measured.
        let removableCaches = systemCaches.children(includeHidden: true)
